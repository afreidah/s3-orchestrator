...
**Overwrites racing their own copies.** An upload that outlives the response can also outlive a later overwrite of the same key. A copy that finishes after a newer write has taken the key cannot be told apart from that write's own copy at the same path, so it is discarded and the copy rebuilt by replication from one the client was told about. When a copy is discarded while another upload for the same key and backend is still in flight, the path is left to that upload and removed only by the last of them to resolve, so a discard cannot delete bytes a copy already in flight is about to record. A write that begins after the discard's transaction is outside that ordering; only per-write storage keys would close that window. `s3o_replication_write_copies_total{outcome="untrusted"}` counts these; a sustained rate means clients are overwriting keys faster than copies finish, and each one costs a rebuild.
