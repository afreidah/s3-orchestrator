...
| **Scrubber** | configurable (default 6h) | Yes | Works through the copies least recently verified, re-hashes them, and discards any whose bytes no longer match the stored `content_hash`. Only runs when `integrity.enabled: true` and `scrubber_interval > 0`, and only on a tick - never at startup, so an interval longer than the process lifetime means it never runs. |
