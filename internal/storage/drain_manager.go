...
func (d *DrainManager) drainOneObject(ctx context.Context, srcBackend ObjectBackend, srcName string, obj *ObjectLocation) bool {
...
func (d *DrainManager) purgeBackendObjects(ctx context.Context, backend ObjectBackend, name string) {
	for {
		objects, err := d.store.ListObjectsByBackend(ctx, name, 100)
		if err != nil {
