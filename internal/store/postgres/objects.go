...
// ListObjectsDelimited returns one delimiter-grouped page via the loose-index
// -scan query (recursive CTE skip-scan) generated by sqlc: keys whose remainder
// after the prefix contains the delimiter collapse into CommonPrefixes, the rest
// are returned as leaf objects. Each recursive step seeks past the whole
// CommonPrefix group in one indexed hop, so cost is proportional to the entries
// emitted, not the keyspace. The query orders/compares object_key under COLLATE
// "C" so grouping and pagination follow S3 byte order (matching SQLite's BINARY
// collation). The delimiter must be non-empty; callers route empty-delimiter
// lists to ListObjects.
