...
type cbDashboardStore struct {
	inner DashboardStore
	cb    *breaker.CircuitBreaker
}

// NewCBDashboardStore returns a CB-protected view typed as DashboardStore.
func NewCBDashboardStore(inner DashboardStore, cb *breaker.CircuitBreaker) DashboardStore {
	return &cbDashboardStore{inner: inner, cb: cb}
}

// GetQuotaStats forwards to the inner store under the breaker.
func (c *cbDashboardStore) GetQuotaStats(ctx context.Context) (map[string]QuotaStat, error) {
...
func (c *cbDashboardStore) GetUsageForPeriod(ctx context.Context, period string) (map[string]UsageStat, error) {
	return breaker.CBCall(c.cb, func() (map[string]UsageStat, error) { return c.inner.GetUsageForPeriod(ctx, period) })
}

// ListDirectoryChildren forwards to the inner store under the breaker.
func (c *cbDashboardStore) ListDirectoryChildren(ctx context.Context, prefix, startAfter string, maxKeys int) (*DirectoryListResult, error) {
	return breaker.CBCall(c.cb, func() (*DirectoryListResult, error) {
