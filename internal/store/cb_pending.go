...
func NewCBPendingStore(inner PendingStore, cb *breaker.CircuitBreaker) PendingStore {
	return &cbPendingStore{inner: inner, cb: cb}
}

// InsertPending forwards to the inner store under the breaker.
func (c *cbPendingStore) InsertPending(ctx context.Context, p *PendingObject) error {
...
func (c *cbPendingStore) GetStalePending(ctx context.Context, olderThan time.Time, limit int) ([]PendingObject, error) {
	return breaker.CBCall(c.cb, func() ([]PendingObject, error) {
...
func (c *cbPendingStore) PromotePending(ctx context.Context, p *PendingObject) (PendingPromoteResult, []DeletedCopy, error) {
