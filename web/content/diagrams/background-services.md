...
(function() {
  ...
  var nodeInfo = {
    ...
    RECONCILE: {
      ...
      body: '<p><code>Reconciler.Run()</code> scans each backend via <code>ReconcileBackend()</code>, which performs a full diff in a single <code>ListObjects</code> call per backend: imports untracked objects and removes stale DB entries where objects no longer exist on the backend.</p><p><b>Interval</b>: configurable (default 24 hours).<br><b>Advisory lock</b>: <code>LockReconcile = 1009</code>.<br><b>Guard</b>: only runs when <code>reconcile.enabled: true</code>.<br><b>On-demand</b>: also available via <code>POST /admin/api/reconcile[?backend=name]</code>.</p><p>For each backend, lists all S3 objects, diffs against <code>object_locations</code>, imports untracked objects via <code>ImportObject()</code>, and deletes stale entries via <code>DeleteObjectLocation()</code>.</p><p>After reconciling, refreshes quota metrics to reflect changes.</p><p>Audit event: <code>storage.ReconcileComplete</code> with imported count, removed count, backends scanned.</p>'
    },
    CBWATCH: {
      title: 'CB Watchdog',
